...
int main(int argv,char** argc)
{
    ...
    cv::Mat img_left = cv::imread("../../img/im2.png", cv::IMREAD_GRAYSCALE);
    cv::Mat img_right = cv::imread("../../img/im6.png", cv::IMREAD_GRAYSCALE);
    ...
    for (uint32 i=0;i<height;i++) {
	    for(uint32 j=0;j<width;j++) {
            ...
            if (disp == Invalid_Float) {
                ...
                disp_mat.data[i * width + j] = 2 * static_cast<uchar>(disp);//目前子像素拟合和之前没有区别，原因在于此强制转换后少数点位置被抹去，并不会出现差别。
            }
	    }
    }
