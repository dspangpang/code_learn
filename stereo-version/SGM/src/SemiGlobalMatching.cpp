    ...
    for (sint32 i = 0; i < height_; i++) {
        for (sint32 j = 0; j < width_; j++) {
            ...
            for (sint32 d = min_disparity; d < max_disparity; d++) {
                ...
                cost_local[d_idx] = cost_ptr[i * width_ * disp_range + j * disp_range + d_idx];
            ...
            const uint16 denom = cost_1 + cost_2 - 2 * min_cost ;
            disp_left_[i * width_ + j] = static_cast<float32>(best_disparity) + static_cast<float32>(cost_1 - cost_2) / (denom * 2.0f);
